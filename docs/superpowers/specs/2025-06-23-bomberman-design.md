...
| `Explosion` | 十字形爆炸视觉动画；驱动持续伤害（active cells）；详见 `2026-06-24-explosion-damage-design.md` |
...
- **玩家输入手感**（单槽缓冲、即时朝向）：见 `docs/superpowers/specs/2026-06-24-player-movement-feel-design.md`。
...
- 连锁反应：炸弹被爆炸波及时立即引爆。
- **爆炸伤害**（详见 `docs/superpowers/specs/2026-06-24-explosion-damage-design.md`）：
  - **引爆瞬间**：`get_explosion_cells` 返回的全部范围内炸弹人立即阵亡。
  - **动画期间**：`Explosion` 每帧对「当前有火焰」的格（`0 < t < 1`）持续判定；走入实火格亦阵亡。
  - 软墙摧毁、道具清除、连锁引爆仅在引爆瞬间处理，不随动画延迟。
